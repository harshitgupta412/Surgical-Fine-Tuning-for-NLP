...
def gen_eda(train_orig, output_file, alpha_sr, alpha_ri, alpha_rs, alpha_rd, num_aug=9):
    ...
    for i, line in enumerate(lines):
        ...
        aug_sentences1, aug_types1 = eda(sentence, alpha_sr=alpha_sr, alpha_ri=alpha_ri, alpha_rs=alpha_rs, p_rd=alpha_rd, num_aug=num_aug)
        # aug_sentences2, aug_types2 = eda(sentence2, alpha_sr=alpha_sr, alpha_ri=alpha_ri, alpha_rs=alpha_rs, p_rd=alpha_rd, num_aug=num_aug)
        for a, aug_type in zip(aug_sentences1, aug_types1):
            writer.write(label + '\t' + a + '\t' +  aug_type + '\t'+ str(i) + '\n')
